...
func S3File(logger *log.Logger, option storage.S3FileOption) (err error) {
	if len(option.AK) == 0 {
	...
	if utils.IsUploadDirection(option.Direction) {
		_, err = storage.S3Upload(option.AK, option.SK, option.Region, option.Endpoint, option.Bucket, option.LocalFile, option.RemoteFile)
	} else if utils.IsDownloadDirection(option.Direction) {
		err = storage.S3Download(option.AK, option.SK, option.Region, option.Endpoint, option.Bucket, option.LocalFile, option.RemoteFile)
	}

...
func Task(logger *log.Logger, option task.TaskOption) (err error) {
	tasks, err := ReadTaskYaml(utils.GetAbsoluteFilePath(option.FilePath))
	if err != nil {
	...
	for _, t := range tasks {
		...
		for _, addr := range hosts {
			...
			c, err := host.NewHostConnection(addr, option.Port, option.Username, option.Password, option.Password)
			if err != nil {
				logger.Error.Println(err)
				continue
			}
...
func RenderFunc(step *v1.Step) (*v1.Step, error) {
	...
	for _, item := range funcStrList {
		...
		if strings.HasPrefix(funcInfo[1], ")") {
			funcResult, err = task.CallMap(funcInfo[0])
		} else {
			...
			funcResult, err = task.CallMap(funcInfo[0], paramsi...)
		}
		if len(funcResult) > 0 {
...
func GetStepFunc(step v1.Step) func(t *v1.Task, c *host.HostConnection, step v1.Step, to task.TaskOption) (string, bool) {
...
func runStepScript(t *v1.Task, c *host.HostConnection, step v1.Step, option task.TaskOption) (result string, isSuccessed bool) {
	stdout, exit, _ := c.Script(option.Sudo, step.Script)
	return stdout, exit == 0
}

func runStepCopy(t *v1.Task, c *host.HostConnection, step v1.Step, option task.TaskOption) (result string, isSuccessed bool) {
